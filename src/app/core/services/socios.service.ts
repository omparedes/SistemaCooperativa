...
import { SocioConPuesto } from '../../pages/socios/socio.model';
...
const DNI_INSTITUCIONAL = 'COOP-00000';
...
export class SociosService {
  ...
  async cargar(): Promise<void> {
    ...
    try {
      ...
      const filas = (data ?? []) as unknown as SocioRow[];
      this._socios.set(filas.map(f => this.mapear(f)));
    } catch (e: unknown) {
  ...
  private mapear(row: SocioRow): SocioConPuesto {
